...
import { PiUser, PiFanLight } from "react-icons/pi";
import { FaUsers } from "react-icons/fa6";
import { TiFilter } from "react-icons/ti";
import { useState , useEffect} from "react"
import { db } from "../../firebase/FirebaseConfig";
import {or, collection, query, getDocs, where, getDoc, doc } from "firebase/firestore";
...
export default function Dashboard() {
  ...
  const fetchActiveBookings = async () => {
    try {
      const q = query(collection(db, "BookedParking"),
      or(
  ...
  return (
    <div className="p-4 h-[calc(100vh-12vh)] overflow-y-auto" >
      ...
      <div className="grid lg:grid-cols-4 gap-3 grid-cols-2 ">
        {cardData.map((item) => (
          <Card key={item.id} icon={item.icon} title={item.title} number={item.number} />
        ))}
      </div>
        ...
        <table className="w-full rounded-2xl bg-white ">
          ...
          <thead>
            <tr className="flex justify-between items-center px-4 w-full text-xl mt-3">
              <h1 className="">New Booking Request</h1>
              <TiFilter
                color="white"
          ...
          <tbody>
            {bookingRequests.map((item, index) => (
              ...
              >
                ...
                <td className= "w-[8%] text-red-500">{item.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
